...
function deleteStudyLog(id){if(!DB.studyLogs)return;DB.studyLogs=DB.studyLogs.filter(l=>l.id!==id);sv('studyLogs');if(window.PAGE==='dashboard')window.renderDashboard(document.getElementById('content-wrap'));else if(window.PAGE==='analytics')window.renderAnalytics(document.getElementById('content-wrap'));}
