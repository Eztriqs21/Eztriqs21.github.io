...
(function() {
  ...
  function chapterRow(ch, subj, index) {
    ...
    return `<div class="list-item anim-entrance" style="--delay:${index * 0.02}s;cursor:pointer" data-tutorial-id="chapter-item" onclick="window.openEditCh('${subj}','${ch.id}')">
      <div class="chapter-check ${ch.completed ? 'done' : ''}" onclick="event.stopPropagation();window.toggleChDone('${subj}','${ch.id}')">
      ...
      <div style="flex:1;min-width:0">
        <div class="list-title" style="${ch.completed ? 'text-decoration:line-through;opacity:0.5' : ''}">${esc(ch.name)}</div>
  ...
  function subjectSection(subjKey) {
    ...
    return `<div class="section-block">
      <div class="section-title" style="cursor:pointer;display:flex;align-items:center;gap:8px" onclick="this.parentElement.classList.toggle('open')">
      ...
      <div class="progress-wrap" style="height:4px;margin-bottom:12px"><div class="progress-bar" style="height:4px;width:${pct}%"></div></div>
      <div style="display:flex;flex-direction:column;gap:4px">
        ${filtered.length === 0 ? `<div class="empty" style="padding:16px"><div class="empty-sub">${_chSearch ? 'No chapters match your search' : 'No chapters yet'}</div></div>` : filtered.map((c, i) => chapterRow(c, subjKey, i)).join('')}
      </div>
    </div>`;
  }
  ...
  window.renderChapters = function(el) {
    ...
    <div class="stats-grid anim-entrance" style="--delay:0.1s">
      <div class="stat-card">
        ...
        <div class="stat-val"><span data-count="${done}">0</span>/<span data-count="${total}">0</span></div>
        <div class="stat-label">Completed</div>
        <div class="stat-sub">${total > 0 ? Math.round((done / total) * 100) : 0}% of syllabus</div>
      </div>
    </div>
