        ...
        <div style="flex:1;padding:16px 18px">
          <div style="display:flex;align-items:flex-start;gap:12px">
            ...
            <div style="flex:1;min-width:0">
              ...
              ${atts.length ? `<div style="display:flex;flex-wrap:wrap;gap:4px;margin-bottom:8px">${atts.map((d, fi) => `<span style="font-size:10px;padding:2px 8px;border-radius:4px;background:var(--border-card);color:var(--text-muted);cursor:pointer" onclick="window.pvFile('${d.data || ''}','${esc(d.name || 'Attachment')}')">${esc(d.name || 'File ' + (fi + 1))}</span>`).join('')}</div>` : ''}
